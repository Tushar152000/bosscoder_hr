...
const OFFICE_LOCATION =
  '2nd Floor, A-150, Sector 63, Noida, UP (near ICICI Bank ATM) — https://maps.app.goo.gl/3W6iucHZimg98HSNA';
const WORKING_DAYS = 'Monday to Friday (WFO), Saturday (WFH)';
...
function buildBody(args: {
  ...
  const signOff = hasPoc ? `${pocName}\n${pocDesignation}` : 'Bosscoder Academy';
  const signOffHtml = hasPoc
    ? `${escapeHtml(pocName)}<br/>${escapeHtml(pocDesignation)}`
    : 'Bosscoder Academy';
  ...
  if (letterKind === 'offer') {
    ...
    const html = `
      ...
      <p style="margin:0 0 14px;font-size:14px;line-height:1.7;color:#374151;">
        Start Date: ${escapeHtml(start)}<br/>
        Location: ${escapeHtml(OFFICE_LOCATION)}<br/>
        Working Days: ${escapeHtml(WORKING_DAYS)}<br/>
        Working Hours: ${escapeHtml(WORKING_HOURS)}
      </p>
      <p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#374151;">
        Please note you will be required to use your personal laptop for work-related tasks.
      </p>
