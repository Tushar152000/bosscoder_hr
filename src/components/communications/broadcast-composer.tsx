...
import { Send, AlertCircle, CheckCircle2, Mail } from 'lucide-react';
...
export function BroadcastComposer({ recipients, selectedDeptCount, senderName, onSent }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl border border-[#E2E8F0] bg-white shadow-sm p-4 sm:p-5 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[12px] font-medium text-slate-700">Subject <span className="text-red-500">*</span></label>
        <input
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[12px] font-medium text-slate-700">Message <span className="text-red-500">*</span></label>
        <textarea
          value={message} onChange={(e) => setMessage(e.target.value)} required rows={6}
          placeholder={'Write your message here…\n\nRecipients will be greeted by their first name.'}
          className="w-full rounded-md border border-[#E2E8F0] bg-white px-3 py-2.5 text-[16px] sm:text-[13px] text-slate-900 placeholder:text-slate-400 resize-none focus:border-[#0C447C] focus:outline-none focus:ring-2 focus:ring-[#0C447C]/20"
        />
        <p className="text-[11px] text-slate-400">Each email starts with &quot;Hi [First name],&quot; and is signed with your name.</p>
      </div>
      ...
      >
        ...
        {!isPending && (recipients.length === 0 ? 'Select recipients first' : `Send to ${recipients.length} recipient${recipients.length !== 1 ? 's' : ''}`)}
      </Button>
    </form>
  );
}
