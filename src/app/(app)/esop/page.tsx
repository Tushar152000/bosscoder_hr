...
import { getEmployeeByUserUid, getAllEmployeesForTree } from '@/lib/firestore/employees';
import { listGrantsForEmployee, listEsopPlans, listAllGrants, deriveVestingSchedule } from '@/lib/firestore/esop';
...
export default async function EsopPage() {
  ...
  const rawGrants: EsopGrantWithPlan[] = me ? await listGrantsForEmployee(me.employeeId) : [];
  ...
  const currentValue = myGrants.reduce((s, g) => s + g.sharesVested * g.plan.perShareValue, 0);

  // Primary grant (most recent) for timeline/breakdown
  const primary = myGrants[0] ?? null;

  const [plans, allEmployees, allGrants] = isAdmin
    ? await Promise.all([listEsopPlans(), getAllEmployeesForTree(), listAllGrants()])
    : [[], [], []];
    ...
    <div className="mx-auto max-w-[1300px] py-8 space-y-6">
      ...
      <div className="rounded-xl border border-[#bcd2ff] bg-[#EBF3FE] px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[13px] text-[#64748b] max-w-[480px]">
            As a valued team member, you have been granted options to own a share of Bosscoder.
            Options vest annually — the more you grow with us, the more you earn.
          </p>
        </div>
        {hasGrants && (
          <div className="flex flex-col gap-2 shrink-0">
            <div className="rounded-lg bg-[#0C447C] px-4 py-2.5 text-center">
              <p className="text-[10px] font-medium text-[#bcd2ff]">Vested value</p>
              <p className="text-[18px] font-bold text-white tabular-nums">{formatLakh(currentValue)}</p>
            </div>
      ...
      {hasGrants && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <MetricCard label="Vested shares" value={String(totalVested)} sub="exercisable" tone="green" />
          <MetricCard label="Unvested shares" value={String(totalUnvested)} sub="pending" tone="amber" />
          <MetricCard label="Per share value" value={formatINR(primary?.plan.perShareValue ?? 0)} sub={`as of ${primary?.plan.valuationYear ?? '—'}`} tone="blue" />
        </div>
      )}
      ...
      {!hasGrants && (
        <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-10 text-center space-y-2">
          ...
          <p className="text-[14px] font-semibold text-[#0f172a]">No grants assigned yet</p>
          <p className="text-[12px] text-[#64748b]">Your ESOP grants will appear here once assigned by HR.</p>
        </div>
      )}
      ...
      {/* ── Share breakdown + Schedule table ── */}
      {primary && (
        <ShareBreakdown
          sharesGranted={primary.sharesGranted}
...
function MetricCard({
  ...
  label: string; value: string; sub: string; tone?: 'default' | 'green' | 'amber' | 'blue';
}) {
  const valueColor = {
    default: 'text-[#0f172a]',
    green: 'text-[#27500A]',
  ...
  return (
    <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4 space-y-1">
      <p className="text-[10px] font-semibold uppercase tracking-[1px] text-[#64748b]">{label}</p>
