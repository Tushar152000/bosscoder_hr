...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: o ? `${o.candidateName} — Relieving letter` : 'Relieving letter' };
}

export default async function EditRelievingLetterPage({ params }: Props) {
  const user = await requirePermission('manage_offer_letters');
  const { id } = await params;
  const offer = await getOfferLetter(id);
  if (!offer || offer.templateKey !== 'relieving') notFound();
